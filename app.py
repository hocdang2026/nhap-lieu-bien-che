import os, re, sqlite3, secrets, unicodedata, csv, json, zipfile, shutil
...
# IMPORTANT: On Render, set DATA_DIR to a mounted Persistent Disk path such as /var/data/nhaplieu.
# Code updates never overwrite files already present in DATA_DIR.
DATA_DIR = Path(os.environ.get('DATA_DIR', BASE_DIR / 'data')).resolve()
DATA_DIR.mkdir(parents=True, exist_ok=True)
DB_PATH = DATA_DIR / 'app.db'
TEMPLATE_PATH = Path(os.environ.get('EXCEL_TEMPLATE', DATA_DIR / 'template.xlsx')).resolve()
...
def bootstrap_data_dir():
    """Copy bundled seed/template only on first run. Never overwrite persistent user data."""
    bundled_template = BUNDLED_DATA_DIR / 'template.xlsx'
    if not TEMPLATE_PATH.exists() and bundled_template.exists():
        shutil.copy2(bundled_template, TEMPLATE_PATH)
    seed_src = BUNDLED_DATA_DIR / 'initial_accounts_seed.csv'
    seed_dst = DATA_DIR / 'initial_accounts_seed.csv'
    if not seed_dst.exists() and seed_src.exists():
...
def admin_dashboard():
    ...
    storage_info = {
        ...
        'persistent_expected': os.environ.get('PERSISTENT_STORAGE', '').lower() in ('1','true','yes') or str(DATA_DIR).startswith('/var/data'),
        'db_exists': DB_PATH.exists(),
        'template': str(active_template_path()),
    }
    return render_template('admin.html', units=units, stats=stats, fields=fields, storage_info=storage_info)
...
def _backup_manifest():
    ...
    try:
        counts = {
            'departments': conn.execute('SELECT COUNT(*) c FROM departments').fetchone()['c'],
            'users': conn.execute('SELECT COUNT(*) c FROM users').fetchone()['c'],
            'submissions': conn.execute('SELECT COUNT(*) c FROM submissions').fetchone()['c'],
            'dynamic_values': conn.execute('SELECT COUNT(*) c FROM dynamic_values').fetchone()['c'],
            'audit_logs': conn.execute('SELECT COUNT(*) c FROM audit_logs').fetchone()['c'],
        }
        meta = get_template_meta(conn)
    finally:
        conn.close()
    return {
        'format': 'nhap-lieu-bien-che-backup-v1',
...
def _create_backup_bytes():
    """Create a full portable backup in memory: DB + active Excel template + supporting files."""
    # Force SQLite WAL contents into app.db before copying.
    conn = db()
    try:
    ...
    with zipfile.ZipFile(bio, 'w', zipfile.ZIP_DEFLATED) as z:
        if DB_PATH.exists():
            z.write(DB_PATH, 'data/app.db')
        for name in ('active_template.xlsx', 'active_template.xlsm', 'template.xlsx', 'initial_accounts_seed.csv'):
            fp = DATA_DIR / name
            if fp.exists():
                z.write(fp, f'data/{name}')
        if CURRENT_XLSX.exists():
            z.write(CURRENT_XLSX, 'data/current_output.xlsx')
        z.writestr('manifest.json', json.dumps(_backup_manifest(), ensure_ascii=False, indent=2))
    bio.seek(0)
    return bio
...
def download_full_backup():
    bio = _create_backup_bytes()
    filename = f"SAO_LUU_NHAP_LIEU_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
...
def restore_full_backup():
    ...
    if not f or not f.filename:
        flash('Vui lòng chọn file sao lưu .zip.', 'danger')
        return redirect(url_for('admin_dashboard'))
    if not f.filename.lower().endswith('.zip'):
        flash('File khôi phục phải là file .zip được tạo từ chức năng Sao lưu của hệ thống.', 'danger')
        return redirect(url_for('admin_dashboard'))
    raw = f.read()
    if len(raw) > 100 * 1024 * 1024:
        flash('File sao lưu quá lớn.', 'danger')
        return redirect(url_for('admin_dashboard'))
    try:
        with zipfile.ZipFile(BytesIO(raw), 'r') as z:
            names = set(z.namelist())
            if 'manifest.json' not in names or 'data/app.db' not in names:
                raise ValueError('Thiếu manifest.json hoặc data/app.db')
            manifest = json.loads(z.read('manifest.json').decode('utf-8'))
            if manifest.get('format') != 'nhap-lieu-bien-che-backup-v1':
                raise ValueError('Định dạng sao lưu không đúng')
            # Validate SQLite backup before replacing live DB.
            tmp_db = DATA_DIR / f'.restore_{secrets.token_hex(6)}.db'
            tmp_db.write_bytes(z.read('data/app.db'))
            ...
            for name in ('active_template.xlsx', 'active_template.xlsm', 'template.xlsx', 'initial_accounts_seed.csv', 'current_output.xlsx'):
                arc = f'data/{name}'
                if arc in names:
                    (DATA_DIR / name).write_bytes(z.read(arc))
        # Normalize stored template path to this deployment's DATA_DIR.
        conn = db()
        meta = get_template_meta(conn)
        old = meta.get('template_path', '')
        candidates = [DATA_DIR/'active_template.xlsm', DATA_DIR/'active_template.xlsx', DATA_DIR/'template.xlsx']
        chosen = next((x for x in candidates if x.exists()), None)
        if chosen:
            set_template_meta(conn, template_path=str(chosen))
        conn.commit(); conn.close()
        rebuild_current_excel()
        session.clear()
        flash('Khôi phục dữ liệu thành công. Vui lòng đăng nhập lại.', 'success')
        return redirect(url_for('login'))
    except Exception as e:
        flash(f'Không thể khôi phục bản sao lưu: {e}', 'danger')
        return redirect(url_for('admin_dashboard'))
